...
//	}
	...
	public void search() { // 검색 기능 가능한 메서드
		...
		for (ShowDTO allLists : allList) { // 향상된 for 문 사용
			// 메인메뉴에서 try/catch 사용 찾는 없는 경우 검색메서드로 리턴
			if (allLists.getActor().contains(kwd) || allLists.getCategory().contains(kwd)
					|| allLists.getDirector().contains(kwd) || allLists.getTitle().contains(kwd)
					|| allLists.getProgram().contains(kwd)) {
				allLists.showList();
			}// 리스트에 포함된 문자열 비교 if 
...
//	public void review(int pNo) {
//		int i = 1;
//		for (ShowDTO all : allList) {
//			System.out.print(i+" | ");
//			all.showList();
//			i++;
//			System.out.println("---------------------------");
